...
// حتى يتم تحديث الواجهة بشكل صحيح بعد تهيئة الشاشةومايصير لخبطة
        ...
        gamesTable.getTableHeader().setBackground(new Color(204, 204, 255, 255));
        gamesTable.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 13));
    ...
    private void initComponents() {
        ...
        gamesTable.setBackground(new java.awt.Color(255, 255, 255));
        gamesTable.setFont(new java.awt.Font("Tahoma", 0, 15)); // NOI18N
        gamesTable.setForeground(new java.awt.Color(0, 0, 0));
        gamesTable.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        gamesTable.setRowHeight(200);
        jScrollPane1.setViewportView(gamesTable);
        if (gamesTable.getColumnModel().getColumnCount() > 0) {
            gamesTable.getColumnModel().getColumn(0).setPreferredWidth(100);
            gamesTable.getColumnModel().getColumn(4).setPreferredWidth(10);
        }
    ...
    private void addnewitemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addnewitemActionPerformed
        AddScreen add = new AddScreen(this, true, "GAME");
        add.setVisible(true);
//هون عملت زي مراقب للشاشه الادد بحيث اول ما تسكر ينضاف ومو بس ع داتا بيس كمان ع الجي تيبل  
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        ...
        try {
            ...
            int id = Integer.parseInt(gamesTable.getValueAt(selectedRow, 5).toString());
...
//نجيب بيانات الصف الي اختارو الشخص  وبعدها نفتح شاشه ال ادد
        int id = Integer.parseInt(gamesTable.getValueAt(row, 5).toString());
        String name = gamesTable.getValueAt(row, 1).toString();
        String model = gamesTable.getValueAt(row, 2).toString();
        double price = Double.parseDouble(gamesTable.getValueAt(row, 3).toString());
        int stock = Integer.parseInt(gamesTable.getValueAt(row, 4).toString());
        ...
        loadGamesTable();     }//GEN-LAST:event_editActionPerformed
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new GamesScreen().setVisible(true));

    }

   public void loadGamesTable() {
    try {
        con = Connect.connect();
        
        //   سحب المنتجات اللي نوعها GAME فقط
        String sql = "SELECT * FROM PRODUCT WHERE TYPE='GAME'";
        pst = con.prepareStatement(sql);
        rs = pst.executeQuery();
        ...
        while (rs.next()) {
            model.addRow(new Object[]{
                ...
                rs.getString("NAME"),
                rs.getString("MODEL"), // رح تقرأ من خانة الـ Model بالداتا بيز وتنعرض تحت عمود الـ Platform
                rs.getDouble("PRICE"),
                rs.getInt("STOCK"),
                rs.getInt("ID")
            });
    ...
    public javax.swing.JTable gamesTable;
